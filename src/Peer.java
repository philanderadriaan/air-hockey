...
public class Peer {
	...
	private List<PhysicsObject> received;
	...
	private void sendMessage(PeerMessage mesg, InetAddress networkID, int port) throws IOException {
		if (networkID == null)
			networkID = InetAddress.getLocalHost();
	...
	public void setup() {
		...
		if (newNetwork) {
			...
			} while (id == -1 ? !connectToNetwork(ip, port) : !connectToNetwork(ip, port, id));
		}
	}
	
	public long getSuccessorID() {
		return successor != null ? successor.chordID : -1;
	}
	...
	public boolean sendPhysicsObject(PhysicsObject o, long chordID) {
	...
	public synchronized List<PhysicsObject> getReceived() {
		List<PhysicsObject> rv = received;
		received = null;
		return rv;
	}

	private boolean handlePayload(PeerMessage mesg) {
		try {
			if (mesg.nodeIdentifier == -1) {
				synchronized (this) {
					if (received == null)
						received = new LinkedList<PhysicsObject>();
					received.add(mesg.payload);
				}
